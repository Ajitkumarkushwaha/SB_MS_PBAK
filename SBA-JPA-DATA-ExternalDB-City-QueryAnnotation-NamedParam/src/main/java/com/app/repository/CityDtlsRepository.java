...
public interface CityDtlsRepository extends JpaRepository<CityDtlsEntity, String> {
	 ...
	 */
	@Query("SELECT C.allPopulation FROM CityDtlsEntity C WHERE C.cityCode=:cityCode")
	 ...
	 */
	@Query("SELECT C.malePopulation, C.femalePopulation FROM CityDtlsEntity C WHERE C.cityCode=:cityCode")
